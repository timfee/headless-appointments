...
function Appoint(params) {
  ...
  let to = (_b = params.to) != null ? _b : add(/* @__PURE__ */ new Date(), { days: 6 });
  ...
  function createPotentialSlots() {
    ...
    while (endDate <= lastDate) {
      ...
      const slotWeekday = Number.parseInt(formatInTimeZone(slotStart, timeZone, "c")) - 1;
      if (
        // Event needs to begin on a valid day.
        limits.daysAllowed.includes(slotWeekday) && // Starts after or on workday start.
        slotStart >= set(slotStart, { hours: limits.fromHour }) && // Ends before or on workday end
      ...
      if (busySlots) {
        busySlots.forEach((event) => {
          ...
          if (event.start && event.end) {
            const eventStart = add(new Date((_a2 = event.start) != null ? _a2 : ""), {
              minutes: padding != null ? padding : 0
            }), eventEnd = sub(new Date((_b2 = event.end) != null ? _b2 : ""), {
              minutes: padding != null ? padding : 0
            });
  ...
  function getSlots(clientTimezone, busy = true) {
    ...
    console.log(
      slotsResult.map((slot) => {
        return formatInTimeZone(slot.start, timeZone, "hh:mm") + " - " + formatInTimeZone(slot.end, timeZone, "hh:mm");
      })
    );
    return slotsResult;
  }
